...
async def get_trade_by_id(trade_id: UUID, service: dep.TradeServiceDep, user: dep.CurrentUserDep) -> Trade | None:
    return await service.get_by_id(trade_id, user.id)

@router.patch("/trades/{trade_id}", response_model=sc.TradeResponse)
async def update_trade(trade_id: UUID, request: sc.TradeUpdateRequest, service: dep.TradeServiceDep, user: dep.CurrentUserDep) -> Trade:
