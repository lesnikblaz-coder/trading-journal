...
@router.post("/analytics/{system_id}/perfromance/month", response_model=sc.AnalyticsOverviewResponse)
async def monthly_performance(system_id: UUID, service: dep.AnalyticsServiceDep, request: sc.PerformanceRequest, user: dep.CurrentUserDep) -> sc.AnalyticsOverviewResponse:
    return await service.overview_monthly_performance(request.year, request.month, system_id, user.id)